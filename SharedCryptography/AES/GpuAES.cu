...
GpuAES::GpuAES()
{
	...
	GPU_CHECKERROR(cudaMalloc(&d_this, sizeof(GpuAES)));
	GPU_CHECKERROR(
			cudaMemcpy(d_this, this, sizeof(GpuAES), cudaMemcpyHostToDevice));
...
__global__ void gpuSetKey(char *w, int Nk, int wLen, GpuAES *aes)
{
	int tx = threadIdx.x;
	...
	if (tx < 4)
	{
		for (int i = Nk; i < wLen; ++i)
		{
			...
			if (i % Nk == 0)
			{
				...
				col = buffer[1][tx] & 15;
				buffer[2][tx] = aes->d_SBOX[row][col];

				buffer[2][tx] ^= aes->d_RCON[i / Nk][tx];
			}
			else if (Nk > 6 && i % Nk == 4)
			{
...
float GpuAES::setKey(const char *key)
{
	...
	gpuSetKey<<<1, wLen * 4, wLen * 4>>>(w, Nk, wLen, d_this);
...
__global__ void gpuEncrypt(char *in, char *out, char *w, int Nr, GpuAES *aes)
{
	char tmp;
	...
	for (int i = 1; i < Nr; ++i)
	{
		row = (bytes[tx] >> 4) & 15;
		col = bytes[tx] & 15;
		bytes[tx] = aes->d_SBOX[row][col]; // Sub bytes

		col = tx & 3;
		bytes[tx] = bytes[((col << 2) + tx) & 15]; // Shift rows

		tmp = aes->d_MUL02[(unsigned char) buffer[col]]
				^ aes->d_MUL03[(unsigned char) buffer[(col + 1) & 3]]
...
__global__ void gpuDecrypt(char *in, char *out, char *w, int Nr, GpuAES *aes)
{
	char tmp;
	...
	for (int i = Nr - 1; i > 0; --i)
	{
		...
		row = (tmp >> 4) & 15;
		col = tmp & 15;
		tmp = aes->d_INV_SBOX[row][col]; // Invert sub bytes

		bytes[tx] = tmp ^ w[(i << 4) + tx]; // Add round key

		col = tx & 3;
		bytes[tx] = aes->d_MUL0E[(unsigned char) buffer[col]]
				^ aes->d_MUL0B[(unsigned char) buffer[(col + 1) & 3]]
				^ aes->d_MUL0D[(unsigned char) buffer[(col + 2) & 3]]
				^ aes->d_MUL09[(unsigned char) buffer[(col + 3) & 3]]; // Invert mix columns
	}
	...
	out[tx + times * 16] = aes->d_INV_SBOX[row][col] ^ w[tx]; // Add round key
}
