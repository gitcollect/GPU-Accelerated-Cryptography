...
__global__ void gpuEncrypt(char *in, char *out, GpuRSA *rsa)
{
	...
	rsa->extGcd(rsa->d_R, rsa->N, invR);
	rsa->extGcd(rsa->N, rsa->d_R, invP);
		...
		bool *passTest, bool *isPrime, GpuRSA *rsa)
{
	...
	rsa->extGcd(rsa->d_R, s_in, s_invR);
	rsa->extGcd(s_in, rsa->d_R, s_invP);
