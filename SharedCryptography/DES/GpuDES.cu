...
/**
 * Check GPU device, allocate global memory, do not allocate constant memory
 */
GpuDES::GpuDES()
{
	deviceReady = false;
	...
	GPU_CHECKERROR(cudaMalloc(&d_this, sizeof(GpuDES)));
	GPU_CHECKERROR(
			cudaMemcpy(d_this, this, sizeof(GpuDES), cudaMemcpyHostToDevice));
 ...
 */
__global__ void gpuSetKey(char *key, bool *subkey, GpuDES *des)
{
	bool tmp;
	...
	{
		int halfIdx = tx % 28;
		int newHalfIdx = (halfIdx + des->d_SHIFT[i]) % 28;
		...
		if (tx < 48)
		{
			subkey[i * 48 + tx] = bits[des->d_PC2[tx] - 1];
		}
...
float GpuDES::setKey(const char *key)
{
	...
	gpuSetKey<<<1, 64, 64>>>(d_Key, subkey, d_this);
 ...
 * Encrypt on GPU
 */
__global__ void gpuEncrypt(char *in, char *out, bool *subkey, GpuDES *des)
{
	bool tmp;
	...
	for (int i = 0; i < 16; ++i)
	{
		...
		if (tx < 48)
		{
			buffer[tx] = high[des->d_E[tx] - 1] ^ subkey[i * 48 + tx];
		}
		...
		if (tx < 32)
		{
			...
			high[tx] = (des->d_SBOX[tx >> 2][y][x] >> (3 - (tx & 3))) & 1;

			high[tx] = high[des->d_P[tx] - 1] ^ bits[tx];
			bits[tx] = tmp;
		}
 ...
 * Decrypt on GPU
 */
__global__ void gpuDecrypt(char *in, char *out, bool *subkey, GpuDES *des)
{
	bool tmp;
	...
	for (int i = 15; i >= 0; --i)
	{
		...
		if (tx < 48)
		{
			buffer[tx] = bits[des->d_E[tx] - 1] ^ subkey[i * 48 + tx];
		}
		...
		if (tx < 32)
		{
			...
			bits[tx] = (des->d_SBOX[tx >> 2][y][x] >> (3 - (tx & 3))) & 1;

			bits[tx] = bits[des->d_P[tx] - 1] ^ high[tx];
			high[tx] = tmp;
		}
